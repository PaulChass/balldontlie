      ...
      if(bets !== 0)
        ...
        <table className="col-md-10 mx-auto table table-hover table-dark"> <tbody>
         { typeof(bets[0])!=='undefined' && bets.slice(-showNBets).reverse().map((bet) => <tr key={bet.id}>
          ...
          <button class="btn btn-dark col-12 m-2 p-2" style={{opacity:0.8}} onClick={() => props.setMenuItem(0)}>Voir les Paris Dispos </button>
          <button class="btn btn-dark col-12 m-2 p-2" style={{opacity:0.8}} onClick={() => props.setMenuItem(3)}>Miser sur ce Match </button>
          <button class="btn btn-dark col-12 m-2 p-2" style={{opacity:0.8}} onClick={() => props.setMenuItem(4)}>Miser sur un Joueur </button></div>)
